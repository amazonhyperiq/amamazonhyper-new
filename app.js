...
/* =====================================================
   مزامنة الأقسام والمواد — مصدر واحد للواجهة العامة
   يحافظ على ترتيب لوحة الإدارة، ويعالج المعرفات العربية
===================================================== */
...
function resolvePublicCategoryId(raw, list=categories){
  ...
  const normalized=list.find(c=>normalizeCategoryKey(c.id)===nk || normalizeCategoryKey(c.name)===nk);
  if(normalized) return String(normalized.id);
  return categoryAliasId(text) || text;
}
...
function mergePublicCategories(savedCategories, legacyStore){
  ...
  const add=(c)=>{
    if(!c) return;
    const id=String(c.id||""); if(!id) return;
    const key=normalizeCategoryKey(id);
    if(seen.has(key)) return;
    seen.add(key);
    result.push({id,name:String(c.name||"قسم"),icon:String(c.icon||"🛒"),image:c.image?String(c.image):"",unit:c.unit==="وزن (كغ)"?"وزن (كغ)":"قطعة",step:Number(c.step)>0?Number(c.step):1});
  };
  input.forEach(add);
  fallback.forEach(c=>{
    const already=result.some(x=>resolvePublicCategoryId(x.id,result)===c.id || normalizeCategoryKey(x.name)===normalizeCategoryKey(c.name));
    if(!already) add(c);
  });
  (Array.isArray(legacyStore)?legacyStore:[]).forEach(section=>{
    const raw=section?.category||section?.id||"";
    const id=categoryAliasId(raw) || raw;
    if(id && !result.some(x=>String(x.id)===String(id) || normalizeCategoryKey(x.id)===normalizeCategoryKey(id))) {
      const f=fallback.find(x=>x.id===id);
      add(f || {id,name:String(raw),icon:"🛒"});
    }
  });
  return result;
}
...
function mergePublicProducts(savedProducts, legacyStore){
  ...
  const add=(p, fallbackCat)=>{
    ...
    result.push({id:p.id,cat:resolvePublicCategoryId(rawCat),name:p.name,price:Number(p.price)||0,unit:p.unit||"قطعة",step:Number(p.step)>0?Number(p.step):1,image:p.image?String(p.image):"",icon:p.icon||"🛒"});
  };
  (Array.isArray(savedProducts)?savedProducts:[]).forEach(p=>add(p));
  (Array.isArray(legacyStore)?legacyStore:[]).forEach(section=>{
    const raw=section?.category||"food";
    (Array.isArray(section?.products)?section.products:[]).forEach(p=>add(p,raw));
  });
  return result;
}
...
async function loadProductsFromSupabase(){
  ...
  try{
    ...
    categories = mergePublicCategories(saved.categories, saved.store);
...
function renderCategories(){
  ...
  // الحاسبة: عجلة الماوس تحرك الشريط أفقياً، والسحب بالماوس يعمل بدون التأثير على الضغط.
  ...
  el.onpointerdown = (event)=>{
    ...
    dragging=true; moved=false; startX=event.clientX; startScroll=el.scrollLeft;
...
/* مزامنة الواجهة العامة بعد أي تعديل من لوحة الإدارة */
let lastPublicRefresh=0;
function refreshPublicStoreIfNeeded(){
  const now=Date.now();
  if(now-lastPublicRefresh<1500) return;
  lastPublicRefresh=now;
  loadProductsFromSupabase();
}
window.addEventListener("focus", refreshPublicStoreIfNeeded);
window.addEventListener("pageshow", refreshPublicStoreIfNeeded);
document.addEventListener("visibilitychange", ()=>{
  if(document.visibilityState==="visible") refreshPublicStoreIfNeeded();
});
