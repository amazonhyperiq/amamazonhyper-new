...
async function loadProductsFromSupabase(){
  ...
  try{
    ...
    const modernCategories = Array.isArray(saved.categories) ? saved.categories : [];
    const legacyCategoryEntries = Array.isArray(saved.store) ? saved.store : [];
    const categoryMap = new Map();
    const addCategory = (c, fallbackIndex) => {
      const rawName = String(c?.name || c?.category || `قسم ${fallbackIndex + 1}`).trim();
      if(!rawName) return;
      const id = String(c?.id || rawName);
      ...
      categoryMap.set(id, {
        ...
        unit: c?.unit === "وزن (كغ)" ? "وزن (كغ)" : "قطعة",
        step: Number(c?.step) > 0 ? Number(c.step) : 1
      });
    };
    modernCategories.forEach((c,i)=>addCategory(c,i));
    legacyCategoryEntries.forEach((c,i)=>addCategory({category:c?.category},i));
    if(categoryMap.size) categories = Array.from(categoryMap.values());
    ...
    /* دمج بيانات store القديمة حتى لا تختفي المواد إذا تم إنشاء
       data.products جديدة تحتوي على جزء فقط من الكتالوج. */
    if(Array.isArray(saved.store)){
      const legacy = [];
      let fallbackId = 1;
      saved.store.forEach(entry => {
        const rawCat = String(entry?.category || "food").trim();
        const matched = categories.find(c => c.id === rawCat || c.name === rawCat);
        (Array.isArray(entry?.products) ? entry.products : []).forEach((p, idx) => {
          const id = Number(p?.id) > 0 ? Number(p.id) : `legacy-${rawCat}-${idx}-${fallbackId++}`;
          legacy.push({id, cat:matched?.id || rawCat, name:String(p?.name||"مادة"), price:Number(p?.price)||0, unit:p?.unit||matched?.unit||"قطعة", step:Number(p?.step)>0?Number(p.step):1, icon:p?.icon||"🛒", image:p?.image?String(p.image):"", variants:Array.isArray(p?.variants)?p.variants:[]});
        });
      });
      const byId = new Map(legacy.map((p,i)=>[String(p.id),i]));
      dbProducts.forEach(p=>{ const k=String(p.id); if(byId.has(k)) legacy[byId.get(k)]={...legacy[byId.get(k)],...p}; else legacy.push(p); });
      dbProducts.length = 0;
      dbProducts.push(...legacy);
    }
...
function renderProductModalVariants(product){
  ...
  el.hidden=false;
  el.innerHTML=`<div class="product-modal-variants-title">اختر النوع</div><div class="product-modal-variants-grid">${variants.map(v=>`<button type="button" class="product-variant-option${selected&&v.id===selected.id?" is-selected":""}" data-variant-id="${escapeHtml(v.id)}" data-variant-image="${escapeHtml(v.image)}" data-variant-name="${escapeHtml(v.name)}"><img src="${escapeHtml(v.image)}" alt="${escapeHtml(v.name)}"><span>${escapeHtml(v.name)}</span></button>`).join("")}</div>`;
}
function productImageMarkup(product,cls="product-image"){
  const src=productDisplayImage(product);
