...
// توحيد أسماء/معرّفات الأقسام حتى لا يؤدي اختلاف العربية القديمة والجديدة إلى اختفاء المواد.
...
const CANONICAL_CATEGORIES = [
  ...
  {id:"canned-veg",name:"الخضراوات المعلبة",icon:"🥫"},
...
function categoryIdForProduct(raw){
  const key=normalizeCategoryKey(raw);
  if(!key) return "";
  const alias=CATEGORY_ALIASES.get(key);
  if(alias) return alias;
  const exact=categories.find(c=>normalizeCategoryKey(c.id)===key);
  if(exact) return exact.id;
  const byName=categories.find(c=>normalizeCategoryKey(c.name)===key);
  return byName ? byName.id : String(raw).trim();
}

function buildMergedCategories(saved){
  const source=Array.isArray(saved?.categories)?saved.categories:[];
  const result=CANONICAL_CATEGORIES.map(c=>({...c}));
  const used=new Set(result.map(c=>normalizeCategoryKey(c.id)));
  const findMatch=(c)=>result.find(r=>normalizeCategoryKey(r.id)===normalizeCategoryKey(c.id) || normalizeCategoryKey(r.name)===normalizeCategoryKey(c.name));
  source.forEach(c=>{
    if(!c || c.id==null) return;
    const m=findMatch(c);
    if(m){
      if(c.image) m.image=String(c.image);
      if(c.unit) m.unit=c.unit;
      if(Number(c.step)>0) m.step=Number(c.step);
    }else{
      const extra={id:String(c.id),name:String(c.name||"قسم"),icon:String(c.icon||"🛒"),image:c.image?String(c.image):"",unit:c.unit==="وزن (كغ)"?"وزن (كغ)":"قطعة",step:Number(c.step)>0?Number(c.step):1};
      result.push(extra); used.add(normalizeCategoryKey(extra.id));
    }
  });
  // أي قسم يظهر فعلياً في المواد/المخزن يُضاف إن لم يكن ضمن القائمة القياسية.
  const rawCats=[];
  if(Array.isArray(saved?.products)) saved.products.forEach(p=>rawCats.push(p?.cat));
  if(Array.isArray(saved?.store)) saved.store.forEach(g=>rawCats.push(g?.category));
  rawCats.forEach(raw=>{
    const id=categoryIdForProductWithList(raw,result);
    if(!id || result.some(c=>normalizeCategoryKey(c.id)===normalizeCategoryKey(id))) return;
    result.push({id,name:String(raw||"قسم"),icon:"🛒",image:"",unit:"قطعة",step:1});
  });
  return result;
}

function categoryIdForProductWithList(raw,list){
  const key=normalizeCategoryKey(raw); if(!key) return "";
  const alias=CATEGORY_ALIASES.get(key); if(alias) return alias;
  const m=list.find(c=>normalizeCategoryKey(c.id)===key || normalizeCategoryKey(c.name)===key);
  return m?m.id:String(raw).trim();
}
...
async function loadProductsFromSupabase(){
  ...
  try{
    ...
    if(
      ...
      /* ==============================================
       تحميل الأقسام — دمج البيانات الحديثة مع الأقسام القديمة
       ولا نسمح لقائمة ناقصة في data.categories بإخفاء الأقسام.
    ============================================== */
    categories = buildMergedCategories(saved);

    /* ==============================================
       تحميل المنتجات — دمج modern products مع store القديم بدون تكرار
    ============================================== */
    const productMap = new Map();

    const addProduct=(product, rawCat)=>{
      if(!product || product.active===false) return;
      const id = product.id!=null ? String(product.id) : `legacy-${productMap.size+1}`;
      const cat = categoryIdForProductWithList(rawCat ?? product.cat, categories);
      const normalized={
        id: product.id!=null ? product.id : productMap.size+1,
        cat: cat || "food",
        name: product.name || "مادة",
        price: Number(product.price)||0,
        unit: product.unit||"قطعة",
        step: Number(product.step)>0 ? Number(product.step) : 1,
        image: product.image ? String(product.image) : "",
        icon: product.icon || "🛒"
      };
      const old=productMap.get(id);
      // modern products هي المصدر الأساسي، والـstore يكمل ما ينقص فقط.
      if(!old || (rawCat==null && product.image) || (!old.image && normalized.image)) productMap.set(id, normalized);
    };

    if(Array.isArray(saved.products)) saved.products.forEach(p=>addProduct(p,p?.cat));
    if(Array.isArray(saved.store)) saved.store.forEach(group=>{
      if(!Array.isArray(group?.products)) return;
      group.products.forEach(p=>addProduct(p,group.category));
    });
...
function renderCategories(){
  ...
  el.innerHTML=categories.map(c=>`
    <button class="category ${String(c.id)===String(selectedCategory)?"active":""}" data-cat="${escapeHtml(c.id)}" type="button">
      <div class="category-icon">
        ${getCategoryImage(c) ? `<img src="${escapeHtml(getCategoryImage(c))}" alt="${escapeHtml(c.name||"")}" loading="lazy" decoding="async">` : escapeHtml(c.icon||"🛒")}
      </div>
  ...
  el.querySelectorAll(".category").forEach(btn=>btn.addEventListener("click",()=>{
    selectedCategory=categoryIdForProduct(btn.dataset.cat||"");
  ...
  // تمرير الحاسبة بعجلة الماوس أفقياً، من دون تعطيل الضغط أو اللمس.
  if(!el.dataset.wheelBound){
    el.addEventListener("wheel",event=>{
      if(Math.abs(event.deltaY) > Math.abs(event.deltaX)){
        const canScroll=el.scrollWidth>el.clientWidth;
        if(canScroll){event.preventDefault();el.scrollLeft += event.deltaY;}
      }
    },{passive:false});
...
function renderSiteContent(){
  ...
  section.className="section site-info-section";
  section.style.cssText="background:linear-gradient(135deg,#f7fff8,#eef8f1);border:1px solid #d7e9dc;margin-bottom:12px;";

  const heading=document.createElement("div");
  heading.className="section-heading";
  const h2=document.createElement("h2"); h2.textContent=c.welcomeTitle||"أهلاً وسهلاً بكم"; heading.appendChild(h2);
  const p=document.createElement("p"); p.textContent=c.welcomeText||""; heading.appendChild(p);
  section.appendChild(heading);

  const detailsWrap=document.createElement("div");
  detailsWrap.className="site-info-details";

  const how=document.createElement("details"); how.className="site-info-details-box";
  const howSummary=document.createElement("summary"); howSummary.textContent="📌 طريقة استخدام الموقع"; how.appendChild(howSummary);
  const howBody=document.createElement("div"); howBody.className="site-info-details-body"; howBody.textContent=c.howToUse||""; how.appendChild(howBody);
  detailsWrap.appendChild(how);

  const info=document.createElement("details"); info.className="site-info-details-box";
  const infoSummary=document.createElement("summary"); infoSummary.textContent="🚚 معلومات مهمة"; info.appendChild(infoSummary);
  const infoBody=document.createElement("div"); infoBody.className="site-info-details-body";
  infoBody.textContent=[
    c.deliveryPhone?`📞 هاتف التوصيل: ${c.deliveryPhone}`:"",
    c.storeAddress?`📍 العنوان: ${c.storeAddress}`:"",
    c.notesInfo?`📝 الملاحظات: ${c.notesInfo}`:"",
    c.deliveryNote?`⚠️ ${c.deliveryNote}`:""
  ].filter(Boolean).join("\n\n");
  info.appendChild(infoBody); detailsWrap.appendChild(info);
