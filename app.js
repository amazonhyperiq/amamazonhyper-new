...
function categoryIdForProduct(rawCat){
  const raw=String(rawCat ?? "").trim();
  if(!raw) return "";
  const exact=categories.find(c=>String(c.id)===raw);
  if(exact) return exact.id;
  const key=normalizeCategoryKey(raw);
  const byId=categories.find(c=>normalizeCategoryKey(c.id)===key);
  if(byId) return byId.id;
  const byName=categories.find(c=>normalizeCategoryKey(c.name)===key);
  if(byName) return byName.id;
  const aliases={
    [normalizeCategoryKey("قسم اللحوم")]:"meat",
    [normalizeCategoryKey("قسم اللحوم الطازجة")]:"meat",
    [normalizeCategoryKey("قسم الدجاج الفريش")]:"chicken",
    [normalizeCategoryKey("قسم الدجاج فريش")]:"chicken",
    [normalizeCategoryKey("قسم الفواكة والخضار")]:"vegetables",
    [normalizeCategoryKey("قسم الفواكه والخضار")]:"vegetables",
    [normalizeCategoryKey("قسم الرز والمواد الغذائية")]:"food",
    [normalizeCategoryKey("قسم الأجبان والألبان")]:"dairy",
    [normalizeCategoryKey("قسم المخللات والزيتون")]:"deli-pickles",
    [normalizeCategoryKey("قسم المخللات - تباع بالوزن")]:"deli-pickles",
    [normalizeCategoryKey("اجبان والبان تباع بالوزن")]:"dairy",
    [normalizeCategoryKey("قسم البيض الطازج")]:"بيض-المائدة",
    [normalizeCategoryKey("قسم الشاي والقهوة")]:"الشاي-والقهوة"
  };
  const alias=aliases[key];
  return alias && categories.some(c=>String(c.id)===alias) ? alias : raw;
}
...
function cacheBustImage(src){
  ...
  if(/^https?:\/\//i.test(value)){
    try{
      ...
      u.searchParams.set("catalog_v","20260914-2");
      return u.toString();
    }catch(e){}
  }
  return value;
...
function mergePublicCategories(data, modernCategories){
  const result=[];
  const seen=new Set();
  const add=(c)=>{
    if(!c) return;
    ...
    if(!name) return;
    const known=categories.find(x=>x.id===c.id || x.name===name) ||
      (name.includes("الفوا") && name.includes("الخضار") ? categories.find(x=>x.id==="vegetables") : null) ||
      (name.includes("المعلبة") ? categories.find(x=>x.id==="canned-veg") : null);
    const id=String(c.id||known?.id||"").trim();
    if(!id || seen.has(id)) return;
    seen.add(id);
    result.push({id,name:known?.name||name,icon:c.icon||known?.icon||"🛒",image:c.image||known?.image||"",unit:c.unit||known?.unit||"قطعة",step:Number(c.step)>0?Number(c.step):(known?.step||1)});
  };
  (Array.isArray(modernCategories)?modernCategories:[]).forEach(add);
  // Always retain the built-in canonical sections so an incomplete DB categories array
  // cannot hide existing products. Legacy store names are added after canonical sections.
  categories.slice().forEach(add);
  (Array.isArray(data?.store)?data.store:[]).forEach(e=>add({category:e?.category}));
  return result;
}
...
async function loadProductsFromSupabase(){
  ...
  try{
    ...
    if(Array.isArray(saved.categories) || Array.isArray(saved.store)){
      categories = mergePublicCategories(saved, saved.categories || []);
    }
      ...
      saved.products.forEach(
        product => {
          ...
          dbProducts.push({
            ...
            cat:categoryIdForProduct(product.cat),
    ...
    selectedCategory = categoryIdForProduct(selectedCategory) || selectedCategory;
...
function renderCategories(){
  ...
  el.innerHTML=categories.map(c=>`
    <button class="category ${c.id===selectedCategory?"active":""}" data-cat="${escapeHtml(c.id)}" type="button">
      <div class="category-icon">
        ${getCategoryImage(c)
          ? `<img src="${escapeHtml(cacheBustImage(getCategoryImage(c)))}" alt="${escapeHtml(c.name||"")}" loading="lazy">`
          : escapeHtml(c.icon||"🛒")}
      </div>
  ...
  // تمرير الأقسام بالماوس على الحاسبة + عجلة الماوس، بدون التأثير على الهاتف.
  // نثبت الأحداث مرة واحدة حتى لا تتكرر بعد كل إعادة رسم للأقسام.
  if(!el.dataset.scrollReady){
    ...
    el.addEventListener("mousedown", e=>{
      ...
      el.classList.add("category-dragging");
    });
    ...
    window.addEventListener("mouseup", ()=>{
      dragging=false; el.classList.remove("category-dragging");
    });
  }
}
...
// خيارات المادة: صور/ألوان/نكهات مرتبطة بمنتج واحد.
const selectedProductVariant = new Map();
function productVariants(product){
  if(!product || !Array.isArray(product.variants)) return [];
  return product.variants.filter(v => v && String(v.image||v.name||"").trim()).map((v,i)=>({
    name:String(v.name||`الخيار ${i+1}`).trim(),
    image:String(v.image||"").trim(),
    id:String(v.id||i+1)
  }));
}
function activeVariant(product){
  ...
  const wanted=String(selectedProductVariant.get(product.id)||"");
  return variants.find(v=>v.id===wanted) || null;
}
function cartKeyFor(product, variant){
  if(!product) return "";
  return variant ? `${String(product.id)}__VARIANT__${String(variant.id)}` : String(product.id);
}
function parseCartKey(key){
  const raw=String(key||"");
  const marker="__VARIANT__";
  const idx=raw.indexOf(marker);
  if(idx<0) return {productId:raw, variantId:""};
  return {productId:raw.slice(0,idx), variantId:raw.slice(idx+marker.length)};
}
function productForCartKey(key){
  const {productId}=parseCartKey(key);
  return products.find(x => String(x.id) === String(productId));
}
function variantForCartKey(product,key){
  const {variantId}=parseCartKey(key);
  if(!product || !variantId) return null;
  return productVariants(product).find(v=>String(v.id)===String(variantId)) || null;
}
function cartQtyFor(product, variant){
  return Number(cart.get(cartKeyFor(product, variant)) || 0);
}
function productCartQty(product){
  ...
  for(const [key,q] of cart){
    const {productId}=parseCartKey(key);
    if(String(productId)===String(product.id)) total += Number(q)||0;
  }
  return total;
}
function productDisplayImage(product){
  const variant=activeVariant(product);
  return variant?.image || String(product?.image||"").trim();
}
function renderProductModalVariants(product){
  const el=document.getElementById("productModalVariants");
  if(!el) return;
  const variants=productVariants(product);
  if(!variants.length){el.hidden=true;el.innerHTML="";return;}
  const selected=activeVariant(product);
  el.hidden=false;
  el.innerHTML=`<div class="product-modal-variants-title">اختر النوع</div><div class="product-modal-variants-grid">${variants.map(v=>`<button type="button" class="product-variant-option${selected&&v.id===selected.id?" is-selected":""}" data-variant-id="${escapeHtml(v.id)}" data-variant-image="${escapeHtml(cacheBustImage(v.image))}" data-variant-name="${escapeHtml(v.name)}"><img src="${escapeHtml(cacheBustImage(v.image))}" alt="${escapeHtml(v.name)}" loading="lazy" decoding="async" referrerpolicy="no-referrer"><span>${escapeHtml(v.name)}</span></button>`).join("")}</div>`;
}
function productImageMarkup(product,cls="product-image"){
  const src=cacheBustImage(productDisplayImage(product));
  return src?`<img class="${cls}" src="${escapeHtml(src)}" alt="${escapeHtml(product?.name||"مادة")}" loading="lazy" decoding="async" referrerpolicy="no-referrer">`:`<div class="product-placeholder">${escapeHtml(product?.icon||"🛒")}</div>`;
}
function renderProducts(){
  ...
  if(!el.dataset.bound){
    el.addEventListener("click",event=>{
      ...
      if(qty&&el.contains(qty)){
        ...
        const targetProduct=products.find(x=>String(x.id)===String(id));
        const hasVariants=!!targetProduct && productVariants(targetProduct).length>0;
        if(qty.classList.contains("product-qty-plus")||qty.classList.contains("product-quick")){
          if(hasVariants) openProductModal(id);
          else changeQty(id,1);
        }else if(qty.classList.contains("product-qty-minus")){
          if(hasVariants) openProductModal(id);
          else changeQty(id,-1);
        }
        return;
  ...
  el.innerHTML=list.map(p=>{
    const q=productCartQty(p), pid=escapeHtml(p.id), qtyText=Number.isInteger(q)?String(q):q.toFixed(2).replace(/0+$/,'').replace(/\.$/,'');
...
function openProductModal(id){
  ...
  renderProductModalVariants(product);
  ...
  const q=cartQtyFor(product, activeVariant(product)); document.getElementById("productModalQty").textContent=Number.isInteger(q)?String(q):q.toFixed(2).replace(/0+$/ ,'').replace(/\.$/ ,'');
...
function initProductModal(){
  ...
  m?.addEventListener("click",e=>{
    if(e.target===m||e.target.classList.contains("product-modal-backdrop")){closeProductModal();return;}
    const option=e.target.closest("button[data-variant-id]");
    if(option && selectedProductForModal){
      const id=String(option.dataset.variantId||"");
      selectedProductVariant.set(selectedProductForModal.id,id);
      const image=String(option.dataset.variantImage||"");
      document.getElementById("productModalImage").innerHTML=image?`<img class="product-image" src="${escapeHtml(image)}" alt="${escapeHtml(selectedProductForModal.name||"مادة")}">`:productImageMarkup(selectedProductForModal);
      m.querySelectorAll(".product-variant-option").forEach(x=>x.classList.toggle("is-selected",x===option));
      const selectedVariant = activeVariant(selectedProductForModal);
      const selectedQty = cartQtyFor(selectedProductForModal, selectedVariant);
      const qtyEl = document.getElementById("productModalQty");
      if(qtyEl) qtyEl.textContent = Number.isInteger(selectedQty)?String(selectedQty):selectedQty.toFixed(2).replace(/0+$/,'').replace(/\.$/,'');
      const priceEl = document.getElementById("productModalAddPrice");
      if(priceEl) priceEl.textContent = money(Number(selectedProductForModal.price||0)*Math.max(selectedQty,1));
      return;
    }
  });
  document.getElementById("productModalPlus")?.addEventListener("click",()=>{
    if(!selectedProductForModal) return;
    const variants=productVariants(selectedProductForModal);
    if(variants.length && !activeVariant(selectedProductForModal)){ alert("يرجى اختيار النوع أولاً"); return; }
    changeQty(selectedProductForModal.id,1,activeVariant(selectedProductForModal)?.id || "");
    openProductModal(selectedProductForModal.id);
  });
  ...
  document.getElementById("productModalAdd")?.addEventListener("click",()=>{
    if(!selectedProductForModal) return;
    const variant=activeVariant(selectedProductForModal);
    if(productVariants(selectedProductForModal).length && !variant){ alert("يرجى اختيار النوع أولاً"); return; }
    const q=cartQtyFor(selectedProductForModal,variant);
    if(!(q>0)) changeQty(selectedProductForModal.id,1,variant?.id || "");
    openProductModal(selectedProductForModal.id);
  });
}
...
function changeQty(
  ...
  const isWeight = unit.includes("كغ") || unit.includes("وزن") || unit.includes("كيلو") || unit.includes("kg");
  const step = isWeight ? (Number(product.step) > 0 ? Number(product.step) : 1) : 1;
  const variant = variantId ? productVariants(product).find(v=>String(v.id)===String(variantId)) : null;
  const key = cartKeyFor(product, variant);
  const currentQty = Number(cart.get(key) || 0);
  const q = Math.max(0, Number((currentQty + delta * step).toFixed(2)));

  if(q) cart.set(key,q); else cart.delete(key);
...
function updateCart(){
  ...
  for(const [key,q] of [...cart]){
    const p = productForCartKey(key);
    const variant = variantForCartKey(p,key);
    const qty = Number(q) || 0;
    if(!p || qty <= 0 || (parseCartKey(key).variantId && !variant)){
      cart.delete(key);
      continue;
    }
...
function renderCartModal(){
  ...
  for(const [key, q] of cart){
    ...
    const variant = variantForCartKey(p,key);
    const qty = Number(q) || 0;
    if(!p || qty <= 0 || (parseCartKey(key).variantId && !variant)) continue;
    ...
    html += `
      <div class="cart-modal-item">
        <div class="cart-modal-item-info">
          ...
          ${variant?`<span class="cart-variant-name">النوع: ${escapeHtml(variant.name)}</span>`:""}
    ...
    if(!itemsEl.dataset.cartEventsBound){
      itemsEl.addEventListener("click", (event) => {
        ...
        if(button.classList.contains("cart-modal-plus")){
          ...
          changeQty(parsed.productId, 1, parsed.variantId);
...
if(whatsappBtn){
  whatsappBtn.addEventListener("click", async () => {
    ...
    for(const [key,q] of cart){
      ...
      // دعم أكثر من لون/نوع لنفس المادة داخل السلة.
      const p = productForCartKey(key);
      const variant = variantForCartKey(p,key);
      if(!p || (parseCartKey(key).variantId && !variant)) continue;
    ...
    const lines = [
      ...
      "",
      ...orderItems.map(item => `- ${item.name}${item.variantName ? ` (${item.variantName})` : ""}: ${item.quantity} ${item.unit} — ${money(item.total)}`),
      "",
