...
function getCategoryImage(category){
  ...
  return CATEGORY_IMAGE_MAP[id] || PUBLIC_CATEGORY_IMAGE_FALLBACKS?.[id] || "";
}
...
/* =====================================================
   مزامنة عامة آمنة للأقسام والمواد
   لا تغيّر بيانات Supabase؛ توحّد المعرفات القديمة والجديدة
===================================================== */
...
const PUBLIC_CATEGORY_DEFS=[
  ...
  {id:"tissues-paper",name:"الكلينس والورقيات",icon:"🧻"},
  {id:"cleaning-tools",name:"أدوات التنظيف المنزلية",icon:"🧽"},
  {id:"بيض-المائدة",name:"بيض المائدة",icon:"🥚"},
  {id:"الشاي-والقهوة",name:"الشاي والقهوة",icon:"☕"},
  {id:"الخضروات-الورقية-المعلبة",name:"الخضروات الورقية المعلبة",icon:"🥬"},
  {id:"زيت-ودهن-الطبخ-ومعجون-الطماطم",name:"زيت ودهن الطبخ ومعجون الطماطم",icon:"🫗"}
];
const PUBLIC_CATEGORY_ALIASES={
  ...
  [normalizeCategoryKey("قسم الخضراوات المعلبة")]:"canned-veg",
  [normalizeCategoryKey("قسم الخضراوات المعلبة فريش")]:"canned-veg",
  [normalizeCategoryKey("قسم الرز والمواد الغذائية")]:"food",
  [normalizeCategoryKey("قسم الرز والزيت والمعجون")]:"food",
  [normalizeCategoryKey("قسم الأجبان والألبان")]:"dairy",
  [normalizeCategoryKey("قسم المخللات والزيتون")]:"deli-pickles",
  [normalizeCategoryKey("قسم المخللات - تباع بالوزن")]:"legumes-packaging",
  ...
  [normalizeCategoryKey("قسم الخضروات الورقية المعلبة")]:"الخضروات-الورقية-المعلبة",
  [normalizeCategoryKey("قسم زيت ودهن الطبخ ومعجون الطماطم")]:"زيت-ودهن-الطبخ-ومعجون-الطماطم",
  [normalizeCategoryKey("قسم مواد التنظيف")]:"cleaning",
  [normalizeCategoryKey("مواد التنظيف المنزلية")]:"cleaning"
};
const PUBLIC_CATEGORY_IMAGE_FALLBACKS={
  "بيض-المائدة":CATEGORY_IMAGE_MAP.dairy,
  "الشاي-والقهوة":CATEGORY_IMAGE_MAP.food,
  "الخضروات-الورقية-المعلبة":CATEGORY_IMAGE_MAP.vegetables,
  "زيت-ودهن-الطبخ-ومعجون-الطماطم":CATEGORY_IMAGE_MAP.food
};
function resolvePublicCategoryId(raw,list=categories){
  const text=String(raw ?? "");
  const exact=list.find(c=>String(c.id)===text || String(c.name)===text);
  if(exact) return String(exact.id);
  const nk=normalizeCategoryKey(text);
  const same=list.find(c=>normalizeCategoryKey(c.id)===nk || normalizeCategoryKey(c.name)===nk);
  if(same) return String(same.id);
  return PUBLIC_CATEGORY_ALIASES[nk] || text;
}
function mergePublicCategories(savedCategories,legacyStore){
  const input=Array.isArray(savedCategories)?savedCategories:[];
  const result=[]; const seen=new Set();
  const add=(c)=>{
    if(!c) return;
    let id=String(c.id||"").trim();
    const alias=PUBLIC_CATEGORY_ALIASES[normalizeCategoryKey(id)];
    if(alias) id=alias;
    if(!id) return;
    ...
    const canonical=PUBLIC_CATEGORY_DEFS.find(x=>x.id===id);
    // إصلاح الأسماء المعروفة التي انقلبت بسبب المعرّف القديم فقط.
    if(canonical && (!name || /كرز/.test(name) || normalizeCategoryKey(name)===normalizeCategoryKey("قسم مواد التنظيف"))) name=canonical.name;
    result.push({id,name:name||canonical?.name||"قسم",icon:String(c.icon||canonical?.icon||"🛒"),image:c.image?String(c.image):"",unit:c.unit==="وزن (كغ)"?"وزن (كغ)":"قطعة",step:Number(c.step)>0?Number(c.step):1,order:Number.isFinite(Number(c.order))?Number(c.order):null});
  };
  input.forEach(add);
  // أي قسم موجود في legacy store نضمن وجوده، لكن لا نغيّر ترتيب الأقسام التي حفظها المدير.
  (Array.isArray(legacyStore)?legacyStore:[]).forEach(section=>{
    const raw=section?.category||section?.id||"";
    const id=PUBLIC_CATEGORY_ALIASES[normalizeCategoryKey(raw)] || String(raw);
    if(id && !result.some(c=>normalizeCategoryKey(c.id)===normalizeCategoryKey(id))){
      const canonical=PUBLIC_CATEGORY_DEFS.find(x=>x.id===id);
      add(canonical || {id,name:String(raw),icon:"🛒"});
    }
  });
  PUBLIC_CATEGORY_DEFS.forEach(add);
  return result;
}
function applySavedCategoryOrder(list, savedOrder){
  if(!Array.isArray(list) || !list.length) return list || [];
  if(Array.isArray(savedOrder) && savedOrder.length){
    const rank=new Map(savedOrder.map((id,i)=>[normalizeCategoryKey(id),i]));
    return list.slice().sort((a,b)=>{
      const ra=rank.has(normalizeCategoryKey(a.id))?rank.get(normalizeCategoryKey(a.id)):999999;
      const rb=rank.has(normalizeCategoryKey(b.id))?rank.get(normalizeCategoryKey(b.id)):999999;
      return ra-rb;
    });
  }
  return list;
}
function mergePublicProducts(savedProducts,legacyStore){
  const result=[]; const seen=new Set();
  const add=(p,fallbackCat)=>{
    if(!p || p.active===false) return;
    const id=String(p.id??"");
    const key=id || `${normalizeCategoryKey(p.name)}|${Number(p.price)||0}|${normalizeCategoryKey(p.unit||"")}`;
    if(seen.has(key)) return;
    seen.add(key);
    const rawCat=p.cat ?? fallbackCat ?? "food";
    result.push({...p,id:p.id,cat:resolvePublicCategoryId(rawCat),name:String(p.name??""),price:Number(p.price)||0,unit:p.unit||"قطعة",step:Number(p.step)>0?Number(p.step):1,image:p.image?String(p.image):"",icon:p.icon||"🛒"});
  };
  (Array.isArray(savedProducts)?savedProducts:[]).forEach(p=>add(p));
  (Array.isArray(legacyStore)?legacyStore:[]).forEach(section=>{
    (Array.isArray(section?.products)?section.products:[]).forEach(p=>add(p,section?.category||"food"));
  });
  return result;
}
...
async function loadProductsFromSupabase(){
  ...
  try{
    ...
    categories = applySavedCategoryOrder(mergePublicCategories(saved.categories, saved.store), saved.categoryOrder);
    ...
    const dbProducts = mergePublicProducts(saved.products, saved.store);
    products = dbProducts;
...
function renderCategories(){
  ...
  el.innerHTML=categories.map(c=>{
    const img=cacheBustImage(getCategoryImage(c) || PUBLIC_CATEGORY_IMAGE_FALLBACKS[String(c.id)] || "");
    const fallback=escapeHtml(c.icon||"🛒");
    return `<button class="category ${String(c.id)===String(selectedCategory)?"active":""}" data-cat="${escapeHtml(c.id)}" type="button" aria-label="${escapeHtml(c.name||"قسم")}">
      <div class="category-icon">${img?`<img src="${escapeHtml(img)}" alt="${escapeHtml(c.name||"")}" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.outerHTML='<span class=&quot;category-fallback-icon&quot;>${fallback}</span>'">`:fallback}</div>
      <strong>${escapeHtml(c.name||"قسم")}</strong>
    </button>`;
  }).join("");

  // click مستقل عن السحب حتى لا يتعطل على الحاسبة.
  el.onclick=(event)=>{
    const btn=event.target.closest("button.category");
    if(!btn) return;
    selectedCategory=String(btn.dataset.cat||"");
    setProductSearchQuery("");
    renderCategories();
    const title=document.getElementById("productsTitle");
    if(title) title.textContent=getCategoryName(selectedCategory)||"المواد";
    document.getElementById("productsSection")?.scrollIntoView({behavior:"smooth",block:"start"});
  };

  // عجلة الماوس: تحويل الحركة الرأسية إلى أفقية بطريقة مستقلة عن RTL.
  el.onwheel=(event)=>{
  ...
  // سحب بالزر الأيسر على الحاسبة، مع السماح بالضغط العادي.
  let dragging=false,startX=0,startScroll=0,moved=false;
  el.onmousedown=(event)=>{ if(event.button!==0)return; dragging=true;moved=false;startX=event.clientX;startScroll=el.scrollLeft; };
  el.onmousemove=(event)=>{ if(!dragging)return; const dx=event.clientX-startX; if(Math.abs(dx)>6)moved=true; if(moved)el.scrollLeft=startScroll-dx; };
...
function cacheBustImage(src){
  ...
  try{ const u=new URL(value); u.searchParams.set("catalog_v","20260914-final"); return u.toString(); }catch{return value;}
}
function productImageMarkup(product,cls="product-image"){
  const src=cacheBustImage(product?.image||"");
...
function renderSiteContent(){
  const mount=document.getElementById("siteWelcomeMount");
  if(!mount) return;
  const c={...siteContent};
  if(c.enabled===false){mount.innerHTML="";return;}
  mount.innerHTML=`
    <section class="section site-info-collapsed" aria-label="معلومات الموقع">
      <div class="section-heading" style="margin-bottom:0">
        <div><span class="section-kicker">معلومات</span><h2>معلومات مهمة للزبون</h2><p>اضغط على العنوان لعرض التفاصيل</p></div>
      </div>
      <div class="site-details-list">
        <details class="site-detail">
          <summary>📌 طريقة استخدام الموقع</summary>
          <div class="site-detail-body">${escapeHtml(c.howToUse||"").replace(/\n/g,"<br>")}</div>
        </details>
        <details class="site-detail">
          <summary>🚚 معلومات مهمة</summary>
          <div class="site-detail-body">
            <p>📞 هاتف التوصيل: ${escapeHtml(c.deliveryPhone||"")}</p>
            <p>📍 العنوان: ${escapeHtml(c.storeAddress||"")}</p>
            <p>📝 الملاحظات: ${escapeHtml(c.notesInfo||"")}</p>
            <p>⚠️ ${escapeHtml(c.deliveryNote||"")}</p>
          </div>
        </details>
      </div>
    </section>`;
...
// مزامنة الواجهة مع آخر تعديل من لوحة الإدارة بدون إعادة بناء التطبيق.
let publicRefreshBusy = false;
async function refreshPublicData(){
  if(publicRefreshBusy) return;
  publicRefreshBusy = true;
  try{ await loadProductsFromSupabase(); }
